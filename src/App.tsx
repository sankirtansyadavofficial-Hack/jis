...
export default function App() {
  ...
  const [simDeptA, setSimDeptA] = useState('CSE');
  const [simSemA, setSimSemA] = useState('6th');
  const [simStyleA, setSimStyleA] = useState('Visual');
  const [simSubjectsA, setSimSubjectsA] = useState<string[]>(['Web Development', 'Data Structures']);

  const [simDeptB, setSimDeptB] = useState('CSE');
  const [simSemB, setSimSemB] = useState('6th');
  const [simStyleB, setSimStyleB] = useState('Visual');
  const [simSubjectsB, setSimSubjectsB] = useState<string[]>(['Web Development', 'Operating Systems']);
  ...
  // Simulated real-time matchmaking algorithm for unauthenticated preview
  function runLocalSimulation() {
    ...
    setTimeout(() => {
      let score = 15;
      if (simDeptA === simDeptB) score += 20;
      if (simSemA === simSemB) score += 20;
      
      const shared = simSubjectsA.filter(s => simSubjectsB.includes(s));
      if (shared.length > 0) {
        score += Math.min(30, shared.length * 15);
      }
      
      if (simStyleA === simStyleB) {
        score += 10;
      } else {
        const pair = [simStyleA, simStyleB].sort().join('-');
      ...
      if (finalScore >= 80) {
        setSimResultText(`✨ Exceptional Match! Excellent alignment on ${simDeptA === simDeptB ? 'departments,' : ''} study semester, and subjects. Highly recommended to form a study circle!`);
      } else if (finalScore >= 60) {
        setSimResultText(`👍 Good compatibility! Overlap in focus subjects. A productive study group can be established.`);
      } else {
        setSimResultText(`🌱 Moderately aligned. Great opportunity to exchange diverse perspectives and coordinate on shared modules.`);
      }
    }, 1250);
  }
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-300 ${darkMode ? 'dark bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 md:px-6 py-8">
        {!token ? (
          /* ==================== GUEST LANDING VIEW ==================== */
          <div className="space-y-20 -mt-8 animate-fade-in">
            {/* Interactive Hero Section */}
            <section className="relative min-h-[80vh] flex flex-col items-center justify-center rounded-3xl overflow-hidden px-6 md:px-12 py-16 md:py-24 border border-white/20 dark:border-white/5 bg-gradient-to-b from-indigo-50/45 via-slate-50 to-slate-50 dark:from-[#090e25]/60 dark:via-slate-950/80 dark:to-slate-950/80 shadow-2xl">
              
              {/* Dynamic canvas node network background */}
              <ShaderHero />
              
              {/* Sleek radial decorative grid mesh overlays */}
              ...
              <div className="absolute inset-0 bg-radial-gradient from-transparent via-white/40 to-white dark:via-slate-950/45 dark:to-slate-950 pointer-events-none" />

              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center w-full max-w-6xl relative z-10">
                {/* Hero Typography & CTA */}
                <div className="lg:col-span-7 space-y-6 text-left">
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-100/60 dark:bg-indigo-900/20 text-indigo-750 dark:text-indigo-300 font-sans text-[11px] font-bold uppercase tracking-wider animate-pulse-glow">
                    <Sparkles className="w-3.5 h-3.5 text-indigo-500" />
                    Collegiate AI Companion Matcher
                  </div>
                  <h1 className="text-4xl sm:text-5xl md:text-6xl font-display font-extrabold tracking-tight leading-tight text-slate-900 dark:text-white">
                    Unlock the Power <br />
                    of Collaborative <br />
                    <span className="bg-gradient-to-r from-blue-600 via-indigo-500 to-violet-600 dark:from-blue-400 dark:via-indigo-400 dark:to-violet-400 bg-clip-text text-transparent glow-text-indigo">
                      Study Groups at JIS
                    </span>
                  </h1>
                  <p className="text-slate-600 dark:text-slate-300 text-sm sm:text-base leading-relaxed max-w-xl">
                    Welcome to the premium peer-matching portal built exclusively for JIS University & College cohorts. Seamlessly discover classmates, build focus groups, coordinate schedules, and conquer semester syllabi together.
                  </p>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-4 pt-4">
                    <button
                      ...
                      className="w-full sm:w-auto px-7 py-3.5 bg-gradient-to-r from-blue-600 to-indigo-650 hover:from-blue-700 hover:to-indigo-700 text-white font-bold text-sm rounded-xl flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-indigo-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 glow-button"
                    >
                      Enlist Student Profile
                    ...
                    <button
                      ...
                      className="w-full sm:w-auto px-7 py-3.5 bg-white/70 hover:bg-white dark:bg-slate-900/60 dark:hover:bg-slate-900 border border-slate-205 dark:border-slate-800 text-slate-700 dark:text-slate-200 font-bold text-sm rounded-xl flex items-center justify-center cursor-pointer hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 backdrop-blur-sm"
                    >
                      Access Core Portal
                    </button>
                  </div>
                  ...
                  {/* Micro stats banner */}
                  <div className="flex items-center gap-8 pt-6 border-t border-slate-200/50 dark:border-slate-800/50 max-w-md">
                    <div>
                      <span className="text-2xl font-black font-display text-slate-850 dark:text-white block">1.4K+</span>
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Matched Peers</span>
                    </div>
                    <div>
                      <span className="text-2xl font-black font-display text-slate-850 dark:text-white block">480+</span>
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Study Rooms</span>
                    </div>
                    <div>
                      <span className="text-2xl font-black font-display text-slate-850 dark:text-white block">98%</span>
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Precision Rate</span>
                    </div>
                  </div>
                </div>

                {/* Hero Matching visual card */}
                <div className="lg:col-span-5 flex justify-center lg:justify-end">
                  <div className="w-full max-w-sm p-6 rounded-3xl bg-white/60 dark:bg-slate-900/60 border border-white/40 dark:border-slate-800/40 backdrop-blur-xl shadow-2xl animate-float space-y-5">
                    <div className="flex justify-between items-center pb-3 border-b border-slate-100 dark:border-slate-800/60">
                      <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider">Live Connection Tracker</span>
                    ...
                    <div className="space-y-4 relative">
                      ...
                      <div className="flex items-center justify-between bg-white/80 dark:bg-slate-900/80 p-3 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <span className="font-bold text-xs block text-slate-800 dark:text-slate-100">Ananya Sen</span>
                            <span className="text-[9px] text-slate-400">CSE Department • Sem 6</span>
                          </div>
                        </div>
                        <span className="px-2 py-0.5 bg-blue-50 dark:bg-blue-900/30 text-[9px] font-bold text-blue-700 dark:text-blue-300 rounded">Visual Learner</span>
                      </div>

                      {/* Floating matching line illustration */}
                      <div className="flex justify-center -my-2.5 relative z-10">
                        <div className="h-8 w-24 bg-gradient-to-b from-blue-500 to-indigo-500 rounded-full flex items-center justify-center text-white text-[10px] font-black tracking-widest shadow-md">
                          94% MATCH
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between bg-white/80 dark:bg-slate-900/80 p-3 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <span className="font-bold text-xs block text-slate-800 dark:text-slate-100">Priyanka Roy</span>
                            <span className="text-[9px] text-slate-400">ECE Department • Sem 6</span>
                          </div>
                        </div>
                        <span className="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-900/30 text-[9px] font-bold text-indigo-750 dark:text-indigo-300 rounded">Auditory Learner</span>
                      </div>
                    </div>

                    <div className="text-center pt-2">
                      <p className="text-[10px] text-slate-450 italic leading-snug">
                        "Aligned schedules found on Mon, Wed, Fri (Evening) with overlapping subjects: Web Development & Labs."
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-10">
              <div className="text-center space-y-3">
                <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 text-[10px] font-bold uppercase tracking-widest">
                  Integrated Capabilities
                </div>
                <h2 className="text-3xl font-display font-extrabold text-slate-900 dark:text-white">
                  Designed Specially for University Excellence
                </h2>
                <p className="text-sm text-slate-500 max-w-xl mx-auto leading-relaxed">
                  Everything you need to master your syllabus, organize study groups, and maintain consistent focus streaks.
                </p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
                
                {/* Bento Card 1: Match Engine */}
                <div className="glass-card rounded-3xl p-6 flex flex-col justify-between group">
                  <div className="space-y-4">
                    <div className="h-11 w-11 bg-gradient-to-tr from-blue-500 to-indigo-600 text-white rounded-2xl flex items-center justify-center shadow-md">
                      <Sparkles className="w-5.5 h-5.5" />
                    </div>
                    <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">Smart Compatibility Engine</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                      Our university-specific algorithm analyzes course syllabi, semesters, overlapping subject codes, learning style preferences, and study timings to calculate match scores automatically.
                    </p>
                  </div>
                  <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-bold block pt-6 uppercase tracking-widest group-hover:translate-x-1 transition-transform">
                    Advanced AI Matching →
                  </span>
                </div>

                {/* Bento Card 2: Workspace Hub */}
                <div className="glass-card rounded-3xl p-6 flex flex-col justify-between group">
                  <div className="space-y-4">
                    <div className="h-11 w-11 bg-gradient-to-tr from-purple-500 to-indigo-600 text-white rounded-2xl flex items-center justify-center shadow-md">
                      <MessageSquare className="w-5.5 h-5.5" />
                    </div>
                    <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">Modular Study Workspaces</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                      Every study circle gets a dynamic private workspace complete with real-time short-polling discussion streams, notes/presentations sharing boards, and shared schedules coordinator.
                    </p>
                  </div>
                  <span className="text-[10px] text-purple-600 dark:text-purple-400 font-bold block pt-6 uppercase tracking-widest group-hover:translate-x-1 transition-transform">
                    Interactive Workspace →
                  </span>
                </div>

                {/* Bento Card 3: Focus Pomodoro */}
                <div className="glass-card rounded-3xl p-6 flex flex-col justify-between group">
                  <div className="space-y-4">
                    <div className="h-11 w-11 bg-gradient-to-tr from-amber-500 to-orange-600 text-white rounded-2xl flex items-center justify-center shadow-md">
                      <Flame className="w-5.5 h-5.5" />
                    </div>
                    <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">Focus Timers & Streak Badges</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                      Maintain consistent academic discipline! Log study hours with the integrated Pomodoro timer featuring synthesised audio alarms that auto-log points to boost your check-in streaks.
                    </p>
                  </div>
                  <span className="text-[10px] text-orange-600 dark:text-orange-400 font-bold block pt-6 uppercase tracking-widest group-hover:translate-x-1 transition-transform">
                    Streak Gamification →
                  </span>
                </div>
            ...
            <section className="relative p-8 md:p-12 rounded-3xl overflow-hidden border border-slate-200/50 dark:border-slate-700/50 bg-white/80 dark:bg-slate-900/85 backdrop-blur-xl max-w-5xl mx-auto shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.4)]">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                ...
                <div className="lg:col-span-5 space-y-4 text-left">
                  <span className="text-[10px] bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 font-bold px-2 py-0.5 rounded uppercase tracking-wider">
                    Interactive Playground
                  </span>
                  <h3 className="text-2xl font-display font-extrabold text-slate-850 dark:text-white">
                    Simulate Your Match Compatibility
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                    Test our matchmaking formula live before joining. Select preferences for Student A and Student B, then run the simulation to see how our system analyzes academic coordinates!
                  </p>

                  <div className="bg-slate-50/60 dark:bg-slate-950/45 p-4 rounded-2xl border border-slate-150/40 dark:border-slate-850/40 text-xs">
                    <p className="font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5 mb-1.5">
                      <ShieldCheck className="w-4 h-4 text-blue-500" />
                      Algorithmic Metrics:
                    </p>
                    <ul className="space-y-1 text-slate-550 dark:text-slate-400 font-mono text-[10px]">
                      <li>• Department overlap (+20%)</li>
                      <li>• Semester synchronisation (+20%)</li>
                      <li>• Mutual core subjects (+15% each)</li>
                      <li>• Learning styles synergy (+10%)</li>
                    </ul>
                  </div>
                </div>
                ...
                <div className="lg:col-span-5 bg-white dark:bg-slate-900/80 border border-slate-150/30 dark:border-slate-850 p-6 rounded-2xl shadow-xl flex flex-col md:flex-row gap-6 items-center">
                  ...
                  <div className="flex-1 space-y-4 w-full">
                    ...
                    <div className="space-y-2 pb-3 border-b border-slate-100 dark:border-slate-800">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400">Student A</p>
                      <div className="grid grid-cols-2 gap-2 text-[10px]">
                        <div>
                          <label className="block text-slate-400 mb-0.5">Department</label>
                          <select 
                            value={simDeptA} 
                            onChange={(e) => setSimDeptA(e.target.value)}
                            className="w-full bg-slate-50 dark:bg-slate-950 dark:text-slate-200 border border-slate-200 dark:border-slate-850 rounded px-2 py-1"
                          >
                        ...
                        <div>
                          <label className="block text-slate-400 mb-0.5">Semester</label>
                          <select 
                            value={simSemA} 
                            onChange={(e) => setSimSemA(e.target.value)}
                            className="w-full bg-slate-50 dark:bg-slate-950 dark:text-slate-200 border border-slate-200 dark:border-slate-850 rounded px-2 py-1"
                          >
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[10px] mt-1.5">
                        <div>
                          <label className="block text-slate-400 mb-0.5">Learning Style</label>
                          <select 
                            value={simStyleA} 
                            onChange={(e) => setSimStyleA(e.target.value)}
                            className="w-full bg-slate-50 dark:bg-slate-950 dark:text-slate-200 border border-slate-200 dark:border-slate-850 rounded px-2 py-1"
                          >
                        ...
                        <div>
                          ...
                          <select 
                            onChange={(e) => setSimSubjectsA([e.target.value, 'Data Structures'])}
                            className="w-full bg-slate-50 dark:bg-slate-950 dark:text-slate-200 border border-slate-200 dark:border-slate-850 rounded px-2 py-1"
                          >
                    ...
                    <div className="space-y-2">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-400">Student B</p>
                      <div className="grid grid-cols-2 gap-2 text-[10px]">
                        <div>
                          <label className="block text-slate-400 mb-0.5">Department</label>
                          <select 
                            value={simDeptB} 
                            onChange={(e) => setSimDeptB(e.target.value)}
                            className="w-full bg-slate-50 dark:bg-slate-950 dark:text-slate-200 border border-slate-200 dark:border-slate-850 rounded px-2 py-1"
                          >
                            <option value="CSE">CSE</option>
                            <option value="IT">IT</option>
                        ...
                        <div>
                          ...
                          <select 
                            value={simSemB} 
                            onChange={(e) => setSimSemB(e.target.value)}
                            className="w-full bg-slate-50 dark:bg-slate-950 dark:text-slate-200 border border-slate-200 dark:border-slate-850 rounded px-2 py-1"
                          >
                            <option value="2nd">2nd Sem</option>
                            <option value="4th">4th Sem</option>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[10px] mt-1.5">
                        <div>
                          ...
                          <select 
                            value={simStyleB} 
                            onChange={(e) => setSimStyleB(e.target.value)}
                            className="w-full bg-slate-50 dark:bg-slate-950 dark:text-slate-200 border border-slate-200 dark:border-slate-850 rounded px-2 py-1"
                          >
                            <option value="Visual">Visual</option>
                            <option value="Auditory">Auditory</option>
                        ...
                        <div>
                          <label className="block text-slate-400 mb-0.5">Subject</label>
                          <select 
                            onChange={(e) => setSimSubjectsB([e.target.value, 'Data Structures'])}
                            className="w-full bg-slate-50 dark:bg-slate-950 dark:text-slate-200 border border-slate-200 dark:border-slate-850 rounded px-2 py-1"
                          >
                            <option value="Web Development">Web Dev</option>
                            <option value="Discrete Mathematics">Discrete Maths</option>
                    ...
                    <button
                      ...
                      disabled={simulating}
                      className="w-full py-2 bg-gradient-to-r from-blue-600 to-indigo-650 disabled:from-slate-300 disabled:to-slate-400 hover:from-blue-700 text-white font-bold text-xs rounded-lg cursor-pointer transition-all shadow-md flex items-center justify-center gap-1.5"
                    >
                      {simulating ? (
                        <>
                    ...
                    <div className="relative h-28 w-28 flex items-center justify-center">
                      <svg className="absolute w-full h-full transform -rotate-90">
                        ...
                        <circle
                          ...
                          strokeDashoffset={`${2 * Math.PI * 48 * (1 - (simScore || 0) / 100)}`}
                          strokeLinecap="round"
                      ...
                      <div className="z-10">
                        {simulating ? (
                          ...
                          <>
                            <span className="text-3xl font-display font-black text-indigo-600 dark:text-indigo-400">
                              {simScore}%
                            </span>
                            <span className="text-[8px] font-sans font-bold uppercase tracking-widest text-slate-450 block">MATCH RATE</span>
                          </>
                        ) : (
                          <span className="text-xs text-slate-400 font-bold">READY</span>
                        )}
                      </div>
                    </div>

                    {simResultText && !simulating && (
                      <p className="text-[9px] text-slate-500 dark:text-slate-400 leading-snug mt-3">
                        {simResultText}
                      </p>
                    )}
                  </div>
            ...
            {/* University Department Catalog Showcase */}
            <section className="bg-gradient-to-b from-blue-50/40 to-slate-50/40 dark:from-slate-900/20 dark:to-slate-950 rounded-3xl p-8 border border-slate-200/20 dark:border-slate-850/60 max-w-6xl mx-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                <div className="space-y-4 text-left">
                  <span className="text-xs font-bold text-blue-650 dark:text-blue-400 uppercase tracking-widest block">
                    Institutional Cohorts Directory
                  </span>
                  <h3 className="text-3xl font-display font-extrabold text-slate-850 dark:text-white leading-snug">
                    Spanning Computer Science, IT, Electronics, and Electrical tracks
                  </h3>
                  <p className="text-xs text-slate-500 leading-relaxed">
                    Study Matcher coordinates study circles for major engineering, science, and technology branches. Classmates can create dedicated subject lists tailored for semester mid-term exams, assignments, and campus placements.
                  </p>
                  <div className="flex gap-2 flex-wrap text-xs font-medium">
                    <span className="p-2 py-1.5 bg-white/70 dark:bg-slate-900/60 border border-slate-150/45 dark:border-slate-850/60 rounded-xl hover:border-blue-500 transition-all cursor-default">🛡️ CSE Department</span>
                    <span className="p-2 py-1.5 bg-white/70 dark:bg-slate-900/60 border border-slate-150/45 dark:border-slate-850/60 rounded-xl hover:border-blue-500 transition-all cursor-default">💻 IT Engineering</span>
                    <span className="p-2 py-1.5 bg-white/70 dark:bg-slate-900/60 border border-slate-150/45 dark:border-slate-850/60 rounded-xl hover:border-blue-500 transition-all cursor-default">⚡ ECE Cohorts</span>
                    <span className="p-2 py-1.5 bg-white/70 dark:bg-slate-900/60 border border-slate-150/45 dark:border-slate-850/60 rounded-xl hover:border-blue-500 transition-all cursor-default">⚙️ EE Circuits</span>
                  </div>
                </div>

                <div className="relative flex justify-center">
                  <div className="p-6 bg-white dark:bg-slate-900/80 border border-slate-150 dark:border-slate-850 rounded-2xl shadow-xl w-full max-w-sm space-y-4 text-left">
                    <p className="text-xs font-bold text-slate-800 dark:text-slate-150">Active Seed Students Directory</p>
                    <div className="space-y-3">
                      <div className="flex items-center justify-between text-xs pb-3 border-b border-slate-100 dark:border-slate-850/60">
                        <div className="flex items-center gap-2.5">
                          <span className="font-bold h-7 w-7 bg-blue-100 text-blue-800 rounded-full flex items-center justify-center font-display text-xs">AS</span>
                          <div>
                            <span className="font-semibold block text-slate-800 dark:text-slate-200">Ananya Sen</span>
                            <span className="text-[9px] text-slate-400">Computer Science • 6th Sem</span>
                          </div>
                        </div>
                        <span className="text-[9px] text-emerald-600 dark:text-emerald-450 font-bold bg-emerald-50 dark:bg-emerald-950/20 px-2 py-0.5 rounded-full">94% Compatibility</span>
                      </div>
                      <div className="flex items-center justify-between text-xs pb-3 border-b border-slate-100 dark:border-slate-850/60">
                        <div className="flex items-center gap-2.5">
                          <span className="font-bold h-7 w-7 bg-indigo-100 text-indigo-800 rounded-full flex items-center justify-center font-display text-xs">RD</span>
                          <div>
                            <span className="font-semibold block text-slate-800 dark:text-slate-200">Rohit Das</span>
                            <span className="text-[9px] text-slate-400">IT Sector • 4th Sem</span>
                          </div>
                        </div>
                        <span className="text-[9px] text-blue-650 dark:text-blue-400 font-bold bg-blue-50 dark:bg-blue-900/10 px-2 py-0.5 rounded-full">87% Compatibility</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2.5">
                          <span className="font-bold h-7 w-7 bg-purple-100 text-purple-850 rounded-full flex items-center justify-center font-display text-xs">PR</span>
                          <div>
                            <span className="font-semibold block text-slate-800 dark:text-slate-200">Priyanka Roy</span>
                            <span className="text-[9px] text-slate-400">ECE Labs • 6th Sem</span>
                          </div>
                        </div>
                        <span className="text-[9px] text-purple-750 dark:text-purple-400 font-bold bg-purple-50 dark:bg-purple-950/25 px-2 py-0.5 rounded-full">81% Compatibility</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {/* ==================== PREMIUM COLLEGIATE FOOTER ==================== */}
      <footer className="w-full bg-white dark:bg-slate-950 border-t border-slate-250/20 dark:border-slate-900 py-12 text-xs text-slate-450 select-none backdrop-blur-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 pb-10">
            ...
            <div className="md:col-span-4 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="font-display font-extrabold text-sm text-slate-800 dark:text-white uppercase tracking-wider">
                  StudyMatcher Hub
                </span>
              </div>
              <p className="text-[11px] leading-relaxed text-slate-400 dark:text-slate-500 max-w-xs">
                A highly optimized, collegiate matching registry designed exclusively for students of JIS Group of Institutions. Securely pairing academic collaborators.
              </p>
              
              {/* Dynamic Database Connection check widget (visually fixes and asserts database safety!) */}
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-850/40 font-mono text-[10px]">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                </span>
                <span className="text-slate-500 dark:text-slate-400">
                  Database: <strong className="text-emerald-600 dark:text-emerald-450">CONNECTED & PERSISTED</strong>
                </span>
              </div>
            </div>

            {/* Column 2: Academic Network */}
            <div className="md:col-span-3 space-y-3">
              <h4 className="font-display font-bold text-slate-700 dark:text-slate-300 text-xs uppercase tracking-widest">
                Academic Network
              </h4>
              <ul className="space-y-2 text-slate-500 dark:text-slate-450 font-medium">
                <li><a href="https://jisuniversity.ac.in" target="_blank" rel="noopener noreferrer" className="hover:text-indigo-500 transition-colors">JIS University</a></li>
                <li><a href="https://jiscollege.ac.in" target="_blank" rel="noopener noreferrer" className="hover:text-indigo-500 transition-colors">JIS College of Eng.</a></li>
                <li><a href="https://gnit.ac.in" target="_blank" rel="noopener noreferrer" className="hover:text-indigo-500 transition-colors">GNIT Campus</a></li>
              </ul>
            </div>

            {/* Column 3: Rules & Handbooks */}
            <div className="md:col-span-3 space-y-3">
              <h4 className="font-display font-bold text-slate-700 dark:text-slate-300 text-xs uppercase tracking-widest">
                Guidelines & Safety
              </h4>
            ...
            {/* Column 4: Platform Engine */}
            <div className="md:col-span-2 space-y-3">
              <h4 className="font-display font-bold text-slate-700 dark:text-slate-300 text-xs uppercase tracking-widest">
                Engine Info
              </h4>
              <p className="text-[11px] leading-relaxed text-slate-400 dark:text-slate-500">
                Core v2.4.0 (JSON-DB Standard). Powered by React 19, TypeScript, and Tailwind CSS.
              </p>
            </div>
          </div>

...
let dbDir = '';
try {
  // In CommonJS, __dirname is a global string
  dbDir = __dirname;
} catch (e) {
  // In ESM, calculate using import.meta.url
  try {
    const metaUrl = (import.meta as any)?.url;
    if (metaUrl) {
      dbDir = path.dirname(fileURLToPath(metaUrl));
    } else {
      dbDir = process.cwd();
    }
  } catch (err) {
...
try {
  ...
  DB_FILE_PATH = path.join(dbDir, '..', 'db.json');
}
